...
**AI 助理**是一个基于 Spring Boot 3 的个人 AI 连接器，提供 Web UI + 飞书机器人双入口的 AI 对话、日报生成、客户管理、自媒体运营分析、笔记浏览等功能。所有数据存储在文件系统中（JSON + Markdown），无外部数据库依赖。

- **版本**: 2.0.0
- **端口**: 6790
- **语言**: Java 17
- **框架**: Spring Boot 3.4.4 + Thymeleaf
- **构建工具**: Maven
...
| 层次 | 技术 | 用途 |
|------|------|------|
| 后端框架 | Spring Boot 3.4.4 | Web 服务、DI、定时任务 |
| 模板引擎 | Thymeleaf + layout-dialect | 服务端渲染 HTML |
...
│   │   │   ├── ChatSessionService.java  # 对话记录管理
...
│  │  ├── chat/chat_sessions.json     (对话历史)       │     │
│  │  ├── chat/feishu_sessions.json   (飞书会话)       │     │
│  │  ├── 工作/日报/                   (日报文件)       │     │
│  │  ├── 工作/周报/                   (周报文件)       │     │
│  │  ├── 工作/综合日报/               (AI 综合日报)    │     │
...
浏览器 POST /chat  →  ChatController
                        ...
                        │   └─ sessionService.saveMessage() 保存到 chat_sessions.json
                        │
飞书长连接 → FeishuLongConnectionService.handleMessage()
                        ...
                        │   │   ├─ FeishuChatSessionService.buildHistoryContext() 恢复上下文
                        │   │   ├─ openCodeService.createSession() + sendMessage()
                        │   │   └─ FeishuChatSessionService.saveMessage()
                        │   └─ sendReply() → 飞书 API 回复
```
...
- Thymeleaf 模板（classpath:/templates/, .html, 无缓存, UTF-8）
...
### 5.3 对话管理模块 — ChatSessionService

管理 `chat_sessions.json`（笔记库根目录下）。

- `SessionsData`：包含 `current`（当前会话 ID）和 `sessions`（会话列表）
- 向下兼容旧数据：自动为无 mode 的会话和消息补充默认 mode
...
### 5.6 自媒体数据采集 — MediaDataCollectorService
...
### 运行库说明
- 无外部数据库，所有数据持久化为 JSON/Markdown 文件
- 无 Redis 或消息队列依赖
- 前端无构建步骤（纯 CDN 加载 marked.js）

## 10. 关键设计决策

1. **文件系统即数据库**：所有数据以 JSON/Markdown 文件存储，无需数据库运维，适合个人使用场景。

2. **opencode serve 作为 AI 后端**：通过 REST API 调用外部 AI 服务，解耦 AI 模型与业务逻辑。

3. **双入口架构**：Web UI（Thymeleaf SSR）和飞书 Bot（Webhook + WebSocket），满足不同使用场景。

4. **SSE 流式输出**：AI 回复通过 Server-Sent Events 推送，提升对话体验的实时性。

5. **流式 JSON 解析**：OpencodeService 采用后台线程读取 + Jackson streaming parser 实现 AI 回复的增量处理。

6. **AI 驱动数据采集**：自媒体数据采集通过 AI (webfetch) 实现，无需 Scrapy 等爬虫框架。

7. **模式化对话**：knowledge（知识库）和 coding（编程）双模式，通过 `/code` 前缀切换，路由到不同端口的 AI 服务。

8. **会话复用**：优先复用已有的 opencode 会话，减少创建开销；飞书对话持久化到 JSON 文件。
