...
一个完全自包含的 AI 助理应用——基于 **Spring AI 2.0** + **Spring Boot 4.1**，原生集成知识库 AI 对话（LLM 推理、`@Tool` 工具编排、语义向量检索）、编程 AI（Pi CLI 代码自动排查）、多模态图片识别。不需要任何外部 AI 编排服务，`java -jar` 一步启动，你的数据就在你自己的笔记库里。
...
```
┌─────────────────────────────────────────────────────────────┐
              │  AI 助理 (Spring Boot 4.1 + Spring AI 2.0)      │
  │                                                             │
  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────────┐ │
  │  │ 知识库 AI │  │ @Tool    │  │ 语义检索  │  │ 图片识别    │ │
  │  │ SSE 流式  │◄─┤ 工具编排  │◄─┤ Ollama   │  │ 多模态     │ │
  │  │ 多会话    │  │ NoteTools│  │ Embed    │  │ Vision API │ │
  │  └────┬─────┘  └────┬─────┘  └────┬─────┘  └─────┬──────┘ │
  │       │              │             │              │        │
  │       │         ┌────▼─────────────▼──────────────▼─────┐  │
  │       │         │         你的笔记库 (本地)              │  │
...
| 功能 | 说明 | 入口 |
|------|------|------|
| 🤖 **AI 对话** | 多会话 + SSE 流式输出 + 工具编排 + 历史语义召回 | `/chat` |
| 📊 **综合日报** | 自动读取笔记库生成日报，可定时推送至飞书 | `/` |
| 📦 **数据中心** | 多数据集管理，Schema 定义，Excel/JSON/Markdown 导入 | `/datacenter` |
| 🔄 **数据采集器** | 定时采集外部数据，AI 自动解析入库 | `/collector` |
| 🖼️ **图片识别** | 笔记库图片浏览 + 多模态 AI 分析 | `/image-recognition` |
| 📋 **任务看板** | 任务创建、优先级、截止日期管理 | `/tasks` |
| ⏰ **动态提醒** | 定时提醒（如每天 18:00 下班日报），飞书推送 | `/reminders` |
| 📝 **待办清单** | 自动解析笔记库 `代办.md`，按优先级分类 | 首页展示 |
| 🧩 **模块系统** | 自定义业务模块（目录 + Prompt + 数据文件） | `/module` |
| 📂 **文件浏览** | 在线浏览笔记库目录与文件内容 | `/browse` |
| 🤝 **AI 录入代理** | 为其他页面提供 AI 辅助填写接口 | `/api/ai/*` |
| 🧠 **AI 引导** | 功能介绍与使用指引 | `/ai-guide` |
| ⚙️ **配置管理** | Web UI 配置 LLM / 飞书 / 笔记库路径 | `/config` |
| 📜 **操作日志** | 查看系统操作记录 | `/log` |
| 🏥 **健康检查** | 服务状态 API | `/health` |
| 💻 **编程 AI** | 独立飞书机器人 + Pi CLI 代码自动排查，消息驱动分析，卡片回复 | `/coding` |
...
**对话模式**：
- `knowledge` 模式：笔记库知识问答（默认）
- `code` 模式：编程相关辅助

**历史记忆**：
- 当前会话：自动注入最近 3 轮对话
- 跨会话语义召回：通过 Ollama 向量检索，从所有历史对话中找出最相关的 8 轮上下文
...
轻量级任务看板，数据保存在笔记库 `AI/任务/data.json`。
...
| 数据类型 | 存储位置 | 说明 |
|---------|---------|------|
| **业务数据** | 笔记库（用户指定目录） | JSON / Markdown，全部在本地文件系统 |
| **聊天记录** | `memory.db` (SQLite) | `sessions` + `messages` 表 |
| **对话向量** | `memory.db` (SQLite) | `turn_embeddings` 表，Base64 编码的 768 维向量 |
| **数据集** | `memory.db` (SQLite) | `data_center_datasets` + `data_center_records` 表 |
| **配置信息** | `config.json`（配置目录） | LLM 凭据、飞书配置、笔记库路径、模块定义等 |
...
│
├── controller/                         # Web 控制器（共 18 个）
│   ├── ChatController.java             # /chat — AI 对话（SSE 流式）
│   ├── IndexController.java            # / — 日报首页
│   ├── ConfigController.java           # /config — 配置页
...
│   ├── ImageRecognitionController.java # /image-recognition — 图片识别
│   ├── TaskController.java             # /tasks — 任务看板
│   ├── ReminderController.java         # /reminders — 提醒管理
│   ├── ModuleController.java           # /module — 模块管理
│   ├── BrowseController.java           # /browse — 文件浏览
│   ├── DataEditorController.java       # /data-editor — 数据编辑器
│   ├── AiProxyController.java          # /api/ai/* — AI 辅助录入
│   ├── AiGuideController.java          # /ai-guide — AI 引导页
│   ├── LogController.java              # /log — 日志查看
│   ├── HelpController.java             # /help — 帮助页
│   ├── HealthController.java           # /health — 健康检查
│   ├── GlobalModelAdvice.java          # 全局模板变量注入
...
| **v0.4**（当前） | 加入编程 AI（Pi CLI 飞书机器人 + 代码排查），实现知识库 AI + 编程 AI 双 AI 助理 |
