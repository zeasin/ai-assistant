...
一个完全自包含的 AI 助理应用——基于 **Spring AI 2.0** + **Spring Boot 4.1**，原生集成 LLM 推理、工具编排（`@Tool`）、语义向量检索（Ollama）、多模态图片识别。不需要任何外部 AI 编排服务，`java -jar` 一步启动，你的数据就在你自己的笔记库里。
...
| 💬 **飞书对话** | WebSocket 长连接接收消息，AI 自动回复 | 配置页启用 |
