# 笔灵 AI 笔记助理 — 基于 Spring AI 的本地多知识库智能笔记助理
...
基于 Spring AI 2.0 + Spring Boot 4.1 的本地多知识库 AI 笔记助理，支持 @Tool 工具编排（自动读写文件/搜索笔记）、RAG 语义检索（Ollama 本地向量）、飞书 WebSocket 双向集成、DeepSeek/智谱/商汤多模型切换、定时日报生成、任务管理、图片识别，零外部依赖 java -jar 一步启动，所有数据本地存储不上传云端，完美替代 Notion AI / Obsidian AI 插件
...
**一句话**：笔灵 AI 笔记助理就是大脑本身，不是桥梁。

---

## 🧠 重大创新：无 Session 设计

传统 AI 聊天系统（ChatGPT、Notion AI 等）都需要用户手动管理会话——「新建对话」「切换对话」「删除对话」，每次对话都是独立的上下文孤岛。**本系统首创「无 Session 设计」**，彻底消除会话管理这一概念。

### 核心思想

**一个知识库 = 一个连续对话，用户完全感知不到 Session 的存在。**

```
传统设计：
  用户 → 新建会话A → 对话 → 切换会话B → 对话 → 切换会话C → ...
  用户需要管理 N 个会话，记住每段对话的位置

本系统设计：
  用户 → 选择知识库 → 连续对话 → 清空（仅此一次重置）
  用户只看到一条连续的对话流，无需管理会话
```

### 技术亮点

| 维度 | 传统设计 | 无 Session 设计 |
|------|---------|----------------|
| 用户体验 | 需要管理多个会话，认知负担重 | 打开即聊，无需任何管理 |
| 上下文连贯性 | 不同会话间上下文断裂 | 所有历史对话天然连续 |
| 语义召回 | 只能在当前会话内搜索 | 在整个 KB 的完整历史中搜索 |
| 代码复杂度 | 需要会话 CRUD、切换、合并逻辑 | 简化为一对一映射 |
| 数据一致性 | 多会话可能产生孤立数据 | 单一数据源，不会混乱 |
...
近期 3 轮对话（始终包含）
    +
语义搜索命中的历史轮次（Ollama 可用时，排除近期避免重复）
    ↓
合并注入 AI 上下文
```

不依赖 session 切换，不依赖用户手动选择，AI 自动从完整历史中召回最相关的上下文。
...
```
┌─────────────────────────────────────────────────────────────┐
              │  笔灵 AI 笔记助理 v1.0 (Spring Boot 4.1 + Spring AI 2.0)  │
  │                                                             │
  │  ┌──────────────┐  ┌──────────┐  ┌──────────┐  ┌────────┐ │
...
所有 KB 相关接口（对话、任务、提醒、日报、识图等）都需要 `kbId` 参数来指定操作哪个知识库。页面间切换时会自动携带当前 kbId。
...
```
[笔灵 AI 笔记助理]  首页  对话  识图  工作▾  学习▾  编程AI  配置  日志  帮助
                               │      │
                    ┌──────┴──┐│      │┌──────┴──┐
                    │📋 任务   ││      ││📋 学习计划│
                    │🔔 提醒   ││      ││🔔 复习提醒│
...
- 缩略图浏览
- AI 多模态分析：选择 Vision 模型后 AI 返回文字描述/解读
- 分析结果保存到当前 KB 的笔记库中
...
│   ├── IndexController.java            # / — 重定向到 /chat
│   ├── ConfigController.java           # /config — 全局配置页
│   ├── ApiConfigController.java        # /api/config/* — 配置 API
│   ├── KnowledgeBaseController.java    # /kb/* — 知识库页面（含日报生成/提示词）
│   ├── DataCenterPageController.java   # /datacenter — 数据中心页
│   ├── CollectorPageController.java    # /collector — 采集器页
│   ├── ImageRecognitionController.java # /image — 图片识别（需 kbId）
│   ├── TaskController.java             # /tasks — 任务看板（需 kbId）
│   ├── ReminderController.java         # /reminders — 提醒管理（需 kbId）
│   ├── ModuleController.java           # /modules — 模块系统
│   ├── DataController.java             # /api/data/* — 数据 API（需 kbId）
│   ├── AiGuideController.java          # /ai-guide — AI 引导（需 kbId）
...
| 改动 | 说明 |
|------|------|
| **多知识库** | 一个实例管理多个 KB，数据/对话/记忆/模块完全隔离 |
| **kbId 全链路传递** | 所有 KB 相关接口（任务/提醒/模块/日报/识图/AI指南/数据）均从页面参数获取 kbId，不依赖 session |
| **导航自动携带 kbId** | 切换页面时自动带上当前 kbId，如 `/chat?kbId=2` → `/image?kbId=2` |
